...
public interface PetRepository extends JpaRepository<Pet, Long>, JpaSpecificationExecutor<Pet> {
    ...
    @Query(
        value = "select distinct pet from Pet pet left join fetch pet.type left join fetch pet.owner",
        countQuery = "select count(distinct pet) from Pet pet"
    )
    Page<Pet> findAllWithToOneRelationships(Pageable pageable);

    @Query("select distinct pet from Pet pet left join fetch pet.type left join fetch pet.owner")
